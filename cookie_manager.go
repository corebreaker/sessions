...
// GetOptions gets the options used to initilize this CookieManager instance
func (m *CookieManager) GetOptions() CookieMngrOptions {
    return *m.options
}
...
func NewCookieManagerOptions(store Store, o *CookieMngrOptions) *CookieManager {
    // Copy the options pass as parameter, and modify the copy
    // The copy will be integrated to returned instance of CookieManager
    options := &CookieManager{CookieManager: *o}

    // Modify options to get real applied options
    if options.SessIDCookieName == "" {
        options.SessIDCookieName = DEFAULT_SESSION_ID_COOKIE_NAME
    }
    ...
    // Return a CookieManager instance initialized with options
    return &CookieManager{
        store:            store,
        options:          options,
        cookieSecure:     !options.AllowHTTP,
        sessIDCookieName: options.SessIDCookieName,
        cookiePath:       options.CookiePath,
        cookieMaxAgeSec:  int(options.CookieMaxAge.Seconds()),
    }
}
